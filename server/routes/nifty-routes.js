...
export function registerNiftyRoutes(app) {
  ...
  app.get('/api/nifty/callback', async (req, res) => {
    ...
    try { const { code, error: oauthError, error_description } = req.query; if (oauthError) return res.redirect(`${frontendUrl}/?page=integrations&nifty_error=${encodeURIComponent(error_description || oauthError)}`); if (!code) return res.redirect(`${frontendUrl}/?page=integrations&nifty_error=${encodeURIComponent('No authorization code received')}`); await nifty.exchangeCodeForToken(code); res.redirect(`${frontendUrl}/?page=integrations&nifty_connected=true`); } catch (error) { res.redirect(`${frontendUrl}/?page=integrations&nifty_error=${encodeURIComponent(error.message)}`); }
  });
  ...
  app.get('/api/nifty/projects/:projectId/tasks', async (req, res) => { try { const { status, assignee, milestone } = req.query; res.json(await nifty.getTasks(req.params.projectId, { status, assignee, milestone })); } catch (error) { res.status(500).json({ error: error.message }); } });
  app.get('/api/nifty/tasks/:taskId', async (req, res) => { try { res.json(await nifty.getTask(req.params.taskId)); } catch (error) { res.status(500).json({ error: error.message }); } });
  app.post('/api/nifty/projects/:projectId/tasks', async (req, res) => { try { res.json(await nifty.createTask(req.params.projectId, req.body)); } catch (error) { res.status(500).json({ error: error.message }); } });
  app.put('/api/nifty/tasks/:taskId', async (req, res) => { try { res.json(await nifty.updateTask(req.params.taskId, req.body)); } catch (error) { res.status(500).json({ error: error.message }); } });
  app.post('/api/nifty/tasks/:taskId/complete', async (req, res) => { try { res.json(await nifty.completeTask(req.params.taskId)); } catch (error) { res.status(500).json({ error: error.message }); } });
  app.put('/api/nifty/tasks/:taskId/assign', async (req, res) => { try { res.json(await nifty.assignTask(req.params.taskId, req.body.memberIds)); } catch (error) { res.status(500).json({ error: error.message }); } });
  app.get('/api/nifty/projects/:projectId/milestones', async (req, res) => { try { res.json(await nifty.getMilestones(req.params.projectId)); } catch (error) { res.status(500).json({ error: error.message }); } });
  app.post('/api/nifty/projects/:projectId/milestones', async (req, res) => { try { res.json(await nifty.createMilestone(req.params.projectId, req.body)); } catch (error) { res.status(500).json({ error: error.message }); } });
  app.put('/api/nifty/milestones/:milestoneId', async (req, res) => { try { res.json(await nifty.updateMilestone(req.params.milestoneId, req.body)); } catch (error) { res.status(500).json({ error: error.message }); } });
  app.get('/api/nifty/projects/:projectId/messages', async (req, res) => { try { const { limit, offset } = req.query; res.json(await nifty.getMessages(req.params.projectId, { limit: limit ? parseInt(limit) : undefined, offset: offset ? parseInt(offset) : undefined })); } catch (error) { res.status(500).json({ error: error.message }); } });
  app.post('/api/nifty/projects/:projectId/messages', async (req, res) => { try { const { content, taskId } = req.body; res.json(await nifty.createMessage(req.params.projectId, content, { taskId })); } catch (error) { res.status(500).json({ error: error.message }); } });
  app.get('/api/nifty/projects/:projectId/documents', async (req, res) => { try { res.json(await nifty.getDocuments(req.params.projectId)); } catch (error) { res.status(500).json({ error: error.message }); } });
  app.get('/api/nifty/projects/:projectId/members', async (req, res) => { try { res.json(await nifty.getMembers(req.params.projectId)); } catch (error) { res.status(500).json({ error: error.message }); } });
  app.get('/api/nifty/portfolios', async (req, res) => { try { res.json(await nifty.getPortfolios()); } catch (error) { res.status(500).json({ error: error.message }); } });
  app.get('/api/nifty/portfolios/:portfolioId/projects', async (req, res) => { try { res.json(await nifty.getPortfolioProjects(req.params.portfolioId)); } catch (error) { res.status(500).json({ error: error.message }); } });
