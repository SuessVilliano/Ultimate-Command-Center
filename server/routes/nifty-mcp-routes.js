...
export function registerNiftyMcpRoutes(app) {
  app.get('/api/nifty/mcp/status', (req, res) => {
    res.json(niftyMcp.status());
  });

  app.get('/api/nifty/mcp/tools', async (req, res) => {
    try {
      const tools = await niftyMcp.listTools({ refresh: req.query.refresh === 'true' });
      res.json({ tools });
    } catch (error) {
      res.status(500).json({ error: error.message, ...niftyMcp.status() });
    }
  });

  // Generic gateway used by the Command Center agent/orchestrator. The browser
  // never receives the MCP token; all calls stay server-side.
  app.post('/api/nifty/mcp/call', async (req, res) => {
    try {
      ...
      if (!available.some(item => item.name === tool)) {
        return res.status(400).json({ error: `Unknown Nifty MCP tool: ${tool}` });
      }
  ...
  // Pull recent Nifty team conversation activity into the Command Center's
  // existing unified inbox. Nifty remains the canonical conversation source.
  app.post('/api/nifty/mcp/sync-inbox', async (req, res) => {
    try {
      const tools = await niftyMcp.listTools();
      const commTool = tools.find(t => t.name === 'communication_query') ||
        tools.find(t => /communication.*query|message.*list/i.test(t.name));

      if (!commTool) {
        return res.status(501).json({ error: 'This Nifty MCP server does not expose communication/message reads.' });
      }
      ...
      const toolResult = await niftyMcp.callTool(commTool.name, {
        ...
        limit,
        sort: '-createdAt',
        expand: 'author,chat,task,document,file,parentMessage'
      });

      const payload = unwrapToolResult(toolResult);
      const messages = rowsFrom(payload);
      let synced = 0;
      ...
      for (const message of messages) {
        ...
        unifiedInbox.addToInbox({
          ...
          source: 'nifty',
          title: `${author} · ${chatName}`,
          preview: text.slice(0, 240),
          priority: message.taskId ? 2 : 1,
          metadata: {
            niftyMessageId: message.id,
  ...
  // Send a reply/message back to Nifty from Command Center.
  app.post('/api/nifty/mcp/message', async (req, res) => {
    try {
      const tools = await niftyMcp.listTools();
      const commTool = tools.find(t => t.name === 'communication_mutate') ||
        tools.find(t => /communication.*mutate|message.*create/i.test(t.name));
      ...
      if (populated.length !== 1) {
        return res.status(400).json({ error: 'Provide exactly one Nifty message context.' });
      }
