...
/**
 * Check the current status of a single Freshdesk ticket
 */
async function checkFreshdeskTicketStatus(freshdeskId) {
  if (!freshdeskConfig.domain || !freshdeskConfig.apiKey) {
    throw new Error('Freshdesk not configured');
  }
  ...
  const response = await fetch(`${baseUrl}/tickets/${freshdeskId}`, {
    headers: {
      'Authorization': `Basic ${auth}`,
      'Content-Type': 'application/json'
    }
...
export async function runScheduledAnalysis(scheduleName = 'manual') {
  ...
  try {
    ...
    // 8. Sync stale ticket statuses - mark locally "open" tickets as closed if not found in Freshdesk results
    console.log('\n8. Syncing stale ticket statuses...');
    try {
      const fetchedIds = new Set(tickets.map(t => t.id));
      const localOpenTickets = db.getTicketsByStatus([2, 3, 6, 7]);
      let staleCount = 0;
      for (const localTicket of localOpenTickets) {
        if (!fetchedIds.has(localTicket.freshdesk_id)) {
          // This ticket is "open" locally but wasn't returned by Freshdesk - check its actual status
          try {
            const actualStatus = await checkFreshdeskTicketStatus(localTicket.freshdesk_id);
            if (actualStatus && actualStatus !== localTicket.status) {
              db.updateTicketStatus(localTicket.freshdesk_id, actualStatus);
              staleCount++;
              console.log(`   Ticket #${localTicket.freshdesk_id}: ${localTicket.status} → ${actualStatus}`);
            }
          } catch (e) {
            // If we can't check individual ticket, mark as closed (5) since Freshdesk didn't return it
            db.updateTicketStatus(localTicket.freshdesk_id, 5);
            staleCount++;
            console.log(`   Ticket #${localTicket.freshdesk_id}: marked as closed (not found in Freshdesk)`);
          }
          // Rate limit individual ticket checks
          await new Promise(resolve => setTimeout(resolve, 300));
        }
      }
      console.log(`   Updated ${staleCount} stale ticket statuses`);
    } catch (e) {
      console.log('   Stale ticket sync skipped:', e.message);
    }
