...
export function brokerAccounts() {
  ...
  return [
    ...
    { id: 'PUBLIC_LIVE', provider: 'public', environment: 'live', live: true, configured: Boolean(process.env.HYBRID_EXECUTION_API_KEY || process.env.THZ_API_KEY), enabled: true, capabilities: ['gateway','portfolio','positions','orders','history','quotes','options','preflight'], note: 'Public is routed through the Hybrid Execution Gateway. Public broker secrets stay on the gateway, not Command Center.' },
