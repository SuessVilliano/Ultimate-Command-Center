...
function StatusBadge({ connected, configured, label }) {
  if (connected) return <span className="inline-flex items-center gap-1 rounded-full bg-emerald-500/15 px-2 py-1 text-xs text-emerald-400"><CheckCircle2 className="h-3 w-3"/>{label || 'Connected'}</span>;
  if (configured) return <span className="inline-flex items-center gap-1 rounded-full bg-amber-500/15 px-2 py-1 text-xs text-amber-400"><AlertCircle className="h-3 w-3"/>Configured</span>;
...
function Card({ title, subtitle, icon: Icon, status, children, isDark }) {
  return <section className={`rounded-xl border p-5 ${isDark ? 'border-white/10 bg-[#0a0a0f]' : 'border-gray-200 bg-white shadow-sm'}`}>
    <div className="mb-4 flex items-start justify-between gap-3">
      <div className="flex items-center gap-3"><div className="rounded-lg bg-cyan-500/10 p-2"><Icon className="h-5 w-5 text-cyan-400"/></div><div><h3 className={`font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>{title}</h3>{subtitle && <p className="text-xs text-gray-500">{subtitle}</p>}</div></div>
      {status}
    </div>
    {children}
  </section>;
...
export default function Integrations() {
  ...
  const loadNifty = useCallback(async () => {
    try {
      ...
      if (!status?.authenticated) { setNiftyProjects([]); return; }
      const projectsRes = await fetchWithTimeout(`${API_URL}/api/nifty/projects`);
      if (projectsRes.ok) setNiftyProjects(asArray(await safeJson(projectsRes)));
      else {
        const payload = await safeJson(projectsRes);
        setNiftyProjects([]);
        setError(payload?.error ? `Nifty: ${payload.error}` : `Nifty projects returned HTTP ${projectsRes.status}.`);
      }
    } catch (e) { console.warn('Nifty status unavailable:', e?.message); setNiftyProjects([]); }
  }, [fetchWithTimeout]);

  const loadTaskade = useCallback(async () => {
    try { const r = await fetchWithTimeout(`${API_URL}/api/taskade/workspaces`); if (r.ok) setTaskadeWorkspaces(asArray(await safeJson(r))); else setTaskadeWorkspaces([]); }
    catch (e) { console.warn('Taskade status unavailable:', e?.message); setTaskadeWorkspaces([]); }
  }, [fetchWithTimeout]);

  const loadTaskmagic = useCallback(async () => {
    try { const r = await fetchWithTimeout(`${API_URL}/api/taskmagic/mcp/status`); if (r.ok) { const payload = await safeJson(r); setTaskmagicStatus({ configured: !!payload?.configured, connected: !!payload?.connected }); } }
    catch (e) { console.warn('TaskMagic status unavailable:', e?.message); }
  }, [fetchWithTimeout]);

  const loadSync = useCallback(async () => {
    try { const r = await fetchWithTimeout(`${API_URL}/api/sync/status`); if (r.ok) setSyncStatus(await safeJson(r)); }
  ...
  const connectGoogle = async () => {
    ...
    } catch (e) { setError(`Google connection could not start: ${e?.message || 'Unknown error'}`); }
  };

  const connectNifty = async () => {
    try {
      const r = await fetchWithTimeout(`${API_URL}/api/nifty/auth/url`);
      const data = await safeJson(r);
      if (!r.ok || !data?.url) { setError(data?.error || 'Nifty OAuth is not configured on the Command Center API.'); return; }
      window.location.assign(data.url);
    } catch (e) { setError(`Nifty connection could not start: ${e?.message || 'Unknown error'}`); }
  };
  ...
  return <div className="space-y-6 animate-slide-in">
    ...
    {connectionNotice && <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 p-3 text-sm text-emerald-300">{connectionNotice}</div>}
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      ...
      <Card isDark={isDark} title="Nifty PM" subtitle="Work + task source of truth" icon={FolderKanban} status={<StatusBadge connected={!!niftyStatus?.authenticated} configured={!!integrationStatus?.nifty?.configured || !!niftyStatus?.hasAccessToken}/>}> 
        {niftyStatus?.authenticated ? <div><p className={`text-sm ${cardText}`}>{totalProjects} active portfolio project{totalProjects === 1 ? '' : 's'} available</p><div className="mt-3 max-h-48 space-y-2 overflow-y-auto">{niftyProjects.slice(0, 12).map((project, i) => <div key={project?.id || `nifty-${i}`} className={`rounded-lg border px-3 py-2 text-sm ${isDark ? 'border-white/10 bg-white/5 text-gray-300' : 'border-gray-200 bg-gray-50 text-gray-700'}`}>{project?.name || project?.title || 'Untitled project'}</div>)}</div><button onClick={connectNifty} className="mt-3 w-full rounded-lg border border-purple-500/30 px-3 py-2 text-xs font-semibold text-purple-400 hover:bg-purple-500/10">Reconnect Nifty</button></div> : <div className="space-y-3"><p className={`text-sm ${cardText}`}>Nifty is not authenticated in this Command Center session. Connect it so Operations can use live work and task data.</p><button onClick={connectNifty} className="w-full rounded-lg bg-purple-600 px-3 py-2 text-sm font-semibold text-white hover:bg-purple-700"><ExternalLink className="mr-2 inline h-4 w-4"/>Connect Nifty</button></div>}
      </Card>
