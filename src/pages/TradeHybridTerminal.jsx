...
const safeUrl=(candidate,fallback)=>{try{const u=new URL(candidate||fallback),expected=new URL(fallback);return u.protocol==='https:'&&(u.hostname===expected.hostname||u.hostname.endsWith(`.${expected.hostname}`))?u.href:fallback}catch{return fallback}};
const APPS={
  journal:{label:'Hybrid Journal',url:safeUrl(import.meta.env.VITE_HYBRID_JOURNAL_URL,'https://hybridjournal.co'),description:'Journal, signals, analytics, AI review and trading history.'},
  funding:{label:'Hybrid Funding',url:safeUrl(import.meta.env.VITE_HYBRID_FUNDING_URL,'https://hybridfunding.co'),description:'Prop-firm accounts, programs, funding operations and account access.',embed:true},
  abatev:{label:'ABATEV',url:safeUrl(import.meta.env.VITE_ABATEV_URL,'https://abatev.tradehybrid.co'),description:'Trade Hybrid strategy / execution workspace.'},
  kraken:{label:'Kraken Pro',url:'https://pro.kraken.com',description:'Kraken advanced trading terminal.'},
  copy:{label:'Hybrid Copy',url:safeUrl(import.meta.env.VITE_HYBRID_COPY_URL,'https://copy.tradehybrid.co'),description:'Master/follower copy relationships, sizing, risk controls and execution bridges.'}
};
...
export default function TradeHybridTerminal({status,error,loading,onRetest}){
 ...
 return <div className="space-y-4">
  <section className="rounded-2xl border border-cyan-500/20 bg-[#080b12] overflow-hidden">
   ...
   {tab==='overview'?<Overview status={status} error={error} loading={loading} onRetest={onRetest} onOpen={choose}/>:<RemoteTradingApp id={tab} app={APPS[tab]}/>} 
  </section>
  {tab==='overview'&&<TradingProcess/>}
 </div>
}

function Overview({status,error,loading,onRetest,onOpen}){
 ...
 return <div className="p-4 sm:p-5 space-y-4">
  ...
  <div className="grid sm:grid-cols-2 xl:grid-cols-5 gap-3"><Launch title="Journal" text="Signals + AI + history" onClick={()=>onOpen('journal')} icon={BookOpen}/><Launch title="Funding" text="Prop account ops" onClick={()=>onOpen('funding')} icon={Building2}/><Launch title="ABATEV" text="Strategy workspace" onClick={()=>onOpen('abatev')} icon={Activity}/><Launch title="Kraken" text="Broker terminal" onClick={()=>onOpen('kraken')} icon={ExternalLink}/><Launch title="Hybrid Copy" text="Copy engine — validate first" onClick={()=>onOpen('copy')} icon={Copy}/></div>
 </div>
}

function Launch({title,text,onClick,icon:Icon}){return <button onClick={onClick} className="text-left rounded-xl border border-white/10 bg-white/[.03] p-3 hover:bg-white/[.06]"><Icon className="w-4 h-4 text-cyan-300"/><div className="font-semibold text-white text-sm mt-2">{title}</div><div className="text-xs text-gray-500 mt-1">{text}</div></button>}
...
function RemoteTradingApp({id,app}){
 ...
 return <div className="min-h-[520px]"><div className="px-4 py-3 border-b border-white/10 flex flex-wrap items-center justify-between gap-3"><div><div className="font-semibold text-white">{app.label}</div><div className="text-xs text-gray-500 mt-1">{app.description}</div><div className={`text-[11px] mt-1 ${state==='reachable'?'text-emerald-300':state==='checking'?'text-amber-300':'text-gray-500'}`}>{state==='reachable'?'Endpoint reachable':state==='checking'?'Checking endpoint…':'Reachability could not be verified from this browser'}</div></div><div className="flex gap-2">{app.embed&&<button onClick={()=>setFrameKey(x=>x+1)} className="px-3 py-2 text-xs rounded-lg bg-white/5 text-gray-300"><RefreshCw className="w-3.5 h-3.5 inline mr-1"/>Reload</button>}<button onClick={open} className="px-3 py-2 text-xs rounded-lg bg-cyan-500/15 border border-cyan-500/25 text-cyan-100"><ExternalLink className="w-3.5 h-3.5 inline mr-1"/>Open {app.label}</button></div></div>
  {id==='kraken'&&<div className="mx-4 mt-3 rounded-xl border border-amber-500/20 bg-amber-500/[.07] p-3 text-xs text-amber-200">Kraken requires its secure full-page login. Command Center account and position intelligence remains separate and is only shown when the Hybrid MCP/API status check proves the connection.</div>}
  ...
  {app.embed?<div className="relative h-[700px] mt-3 bg-black"><iframe key={`${id}-${frameKey}`} src={app.url} title={app.label} className="absolute inset-0 w-full h-full border-0" allow="clipboard-read; clipboard-write; fullscreen"/></div>:<div className="m-4 min-h-[360px] rounded-2xl border border-cyan-500/15 bg-gradient-to-br from-cyan-950/20 to-purple-950/10 grid place-items-center p-8 text-center"><div><ExternalLink className="w-10 h-10 text-cyan-300 mx-auto"/><h3 className="text-lg font-semibold text-white mt-4">{app.label} opens securely in its own tab</h3><p className="text-sm text-gray-400 mt-2 max-w-xl">This service blocks being embedded inside Command Center. Opening it directly prevents the blank/403 screen and keeps login and trading security intact.</p><button onClick={open} className="mt-5 px-5 py-3 rounded-xl bg-cyan-500/15 border border-cyan-400/30 text-cyan-100">Open {app.label}</button></div></div>}</div>
}
