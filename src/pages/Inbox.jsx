...
function Inbox() {
  ...
  const loadChannels = useCallback(async () => {
    ...
    if (!response.ok) throw new Error('Team workspace unavailable');
  ...
  const loadWorkspace = useCallback(async () => {
    try {
      const responses = await Promise.all([
        fetch(`${API_URL}/api/team/messages?channel=${encodeURIComponent(activeChannel)}`),
        fetch(`${API_URL}/api/team/tasks?limit=50`), fetch(`${API_URL}/api/team/status`)
      ]);
      ...
      setMessages(messageData.messages || []); setTasks(taskData.tasks || []); setStatus(statusData); setConnected(true);
    } catch (error) { console.error(error); setConnected(false); }
  }, [activeChannel]);
  ...
  const sendMessage = async event => {
    ...
    const content = message.trim(); setMessage(''); setSending(true);
    try {
      const response = await fetch(`${API_URL}/api/team/messages`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ content, channelId: activeChannel, userId: 'jamaur' })
      });
      if (!response.ok) throw new Error((await response.json()).error || 'Message failed');
      await loadWorkspace();
    } catch (error) { setMessage(content); window.alert(error.message); } finally { setSending(false); }
  };
  ...
  return (
    <div className="h-[calc(100vh-7rem)] min-h-[620px] grid grid-cols-[220px_minmax(0,1fr)_340px] gap-4 animate-slide-in">
      <aside className="rounded-xl border border-white/10 bg-white/5 overflow-hidden">
        <div className="p-4 border-b border-white/10">
          ...
          <div className={`mt-2 flex items-center gap-1.5 text-xs ${connected ? 'text-green-400' : 'text-red-400'}`}>
            {connected ? <Wifi className="w-3.5 h-3.5" /> : <WifiOff className="w-3.5 h-3.5" />}{connected ? 'Persistent workspace online' : 'Backend disconnected'}
          </div>
        </div>
        <nav className="p-2 space-y-1">
          {channels.map(channel => <button key={channel.id} onClick={() => setActiveChannel(channel.id)} className={`w-full text-left px-3 py-2.5 rounded-lg flex items-center gap-2 ${activeChannel === channel.id ? 'bg-purple-600 text-white' : 'text-gray-400 hover:bg-white/10 hover:text-white'}`}><Hash className="w-4 h-4" /><span className="truncate">{channel.name}</span><span className="ml-auto text-[10px] opacity-60">{channel.message_count}</span></button>)}
        </nav>
      </aside>
      ...
      <main className="rounded-xl border border-white/10 bg-white/5 flex flex-col min-w-0 overflow-hidden">
        <header className="px-5 py-4 border-b border-white/10 flex items-center justify-between">
          <div><h2 className="text-white font-semibold">#{channels.find(channel => channel.id === activeChannel)?.name || activeChannel}</h2><p className="text-xs text-gray-500">{channels.find(channel => channel.id === activeChannel)?.description}</p></div>
          <button onClick={loadWorkspace} className="p-2 rounded-lg hover:bg-white/10 text-gray-400"><RefreshCw className="w-4 h-4" /></button>
        </header>
        <section className="flex-1 overflow-y-auto p-5 space-y-5">
          ...
          {messages.map(item => <article key={item.id} className="flex gap-3"><div className={`w-9 h-9 rounded-lg grid place-items-center shrink-0 ${item.author_type === 'user' ? 'bg-purple-600' : 'bg-cyan-500/15 text-cyan-300'}`}>{item.author_type === 'user' ? 'J' : <Bot className="w-5 h-5" />}</div><div className="min-w-0"><div className="flex items-baseline gap-2"><span className="text-sm font-semibold text-white">{item.author_name}</span><span className="text-[11px] text-gray-600">{new Date(item.created_at).toLocaleString()}</span></div><p className="mt-1 text-sm text-gray-300 whitespace-pre-wrap leading-6">{item.content}</p>{item.metadata?.status && <span className={`inline-block mt-2 px-2 py-1 rounded text-[11px] ${STATUS_STYLES[item.metadata.status] || 'bg-white/10 text-gray-300'}`}>{item.metadata.status.replaceAll('_', ' ')}</span>}</div></article>)}
          <div ref={bottomRef} />
        </section>
        <form onSubmit={sendMessage} className="p-4 border-t border-white/10"><div className="flex gap-2 rounded-xl bg-black/20 border border-white/10 p-2 focus-within:border-purple-500/60"><textarea value={message} onChange={event => setMessage(event.target.value)} onKeyDown={event => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); sendMessage(event); } }} placeholder="Assign work, ask for an update, or message the team..." rows={2} className="flex-1 resize-none bg-transparent text-sm text-white placeholder-gray-600 outline-none px-2 py-1" /><button disabled={sending || !message.trim()} className="self-end p-2.5 rounded-lg bg-purple-600 hover:bg-purple-500 disabled:opacity-40 text-white">{sending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />}</button></div></form>
      </main>
