...
function dueValue(task) { return task?.dueAt || task?.dueDate || task?.due_date || null; }
...
function projectFor(task) {
  return task?._project || task?.project || { id: task?.projectId || 'nifty', name: task?.project?.name || 'Nifty' };
}
...
export default function NiftyTasks() {
  ...
  const load = async () => {
    ...
    try {
      const live = await fetch(`${API_URL}/api/nifty/mcp/tasks?includeCompleted=true&limit=250`);
      if (live.ok) {
        const payload = await live.json();
        const liveTasks = rows(payload).filter(t => t?.archived !== true).map(t => ({...t, _project: projectFor(t)}));
        setTasks(liveTasks);
        return;
      }
      setTasks(await loadLegacy());
    } catch (e) {
      try { setTasks(await loadLegacy()); }
      catch { setError(e.message || 'Could not load Nifty tasks'); }
    } finally { setLoading(false); }
  };
  useEffect(()=>{ load(); },[]);

  const projects = useMemo(()=>{
    const map=new Map(); tasks.forEach(t=>{const p=projectFor(t);if(p?.id) map.set(p.id,p.name||'Project');});
    return [...map.entries()].sort((a,b)=>a[1].localeCompare(b[1]));
  },[tasks]);

  const visible = useMemo(()=>tasks.filter(t=>{
    const p=projectFor(t);
    if(projectFilter!=='all' && p?.id!==projectFilter) return false;
    if(statusFilter==='open' && t.completed) return false;
    if(statusFilter==='completed' && !t.completed) return false;
    const hay=[t.name,t.title,t.description,p?.name,statusName(t),listName(t),assigneeNames(t)].filter(Boolean).join(' ').toLowerCase();
    return !query || hay.includes(query.toLowerCase());
  }).sort((a,b)=>{
    const ad=dt(dueValue(a)), bd=dt(dueValue(b));
  ...
  const complete = async (task) => {
    ...
    let r=await fetch(`${API_URL}/api/nifty/mcp/tasks/${encodeURIComponent(task.id)}/complete`,{method:'POST'});
    if(!r.ok) r=await fetch(`${API_URL}/api/nifty/tasks/${encodeURIComponent(task.id)}/complete`,{method:'POST'});
  ...
  return <div className="space-y-5 max-w-7xl">
    <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-4">
      <div><div className="text-xs uppercase tracking-[.18em] text-teal-400">Nifty source of truth</div><h1 className={`text-2xl font-bold ${isDark?'text-white':'text-gray-900'}`}>Tasks</h1><p className="text-sm text-gray-500 mt-1">Live tasks from Nifty MCP with legacy fallback. No local duplicate task database.</p></div>
    ...
    <div className={`rounded-xl border p-3 grid md:grid-cols-[1fr_220px_160px] gap-2 ${card}`}>
      ...
      <select value={projectFilter} onChange={e=>setProjectFilter(e.target.value)} className="px-3 py-2 rounded-lg bg-black/20 border border-white/10 text-sm text-gray-300"><option value="all">All Nifty projects</option>{projects.map(([id,name])=><option key={id} value={id}>{name}</option>)}</select>
    ...
    <div className="grid sm:grid-cols-4 gap-3">
      {[['Open',tasks.filter(t=>!t.completed).length],['Completed',tasks.filter(t=>t.completed).length],['Due dated',tasks.filter(t=>dueValue(t)).length],['Visible',visible.length]].map(([l,v])=><div key={l} className={`rounded-xl border p-4 ${card}`}><div className={`text-2xl font-bold ${isDark?'text-white':'text-gray-900'}`}>{v}</div><div className="text-xs text-gray-500">{l}</div></div>)}
    </div>
    ...
    <div className="space-y-3">{visible.map(task=>{
      const due=dueValue(task); const overdue=due && !task.completed && dt(due)<new Date(); const desc=task.description||task.notes||''; const project=projectFor(task);
      return <article key={task.id} className={`rounded-xl border p-4 ${card}`}>
        <div className="flex gap-3">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex flex-wrap items-start justify-between gap-2"><div><h3 className={`font-semibold ${task.completed?'line-through text-gray-500':isDark?'text-white':'text-gray-900'}`}>{task.name||task.title||'Untitled task'}</h3><div className="text-xs text-teal-400 mt-1 flex items-center gap-1"><FolderKanban className="w-3 h-3"/>{project?.name||'Nifty'}</div></div><span className="text-[11px] px-2 py-1 rounded-full border border-white/10 bg-white/5 text-gray-300">{statusName(task)}</span></div>
