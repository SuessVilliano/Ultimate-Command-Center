...
import { compareAffiliateBooks, parseAffiliateBookCSV } from '../../lib/affiliate-book-import';
import {
  BOOK_UPDATED_EVENT,
...
function changeText(diff) {
  if (!diff) return '';
  const c = diff.counts || diff;
  if (!c.added && !c.removed && !c.changed) return 'No roster/data changes detected';
  return `${c.added || 0} added · ${c.removed || 0} removed · ${c.changed || 0} changed`;
}
...
export default function AffiliateDataImport({ isDark = true, buttonClass = '' }) {
  ...
  const refresh = async () => {
    const currentRows = getAffiliateBookRows();
    const currentMeta = getAffiliateBookMeta();
    setBook(currentRows);
    setMeta(currentMeta);
    if (currentMeta?.diff) setLastDiff(currentMeta.diff);
    const nextCloud = await cloudStateStatus().catch(() => ({ configured: false, connected: false }));
    setCloud(nextCloud);
    if (nextCloud.connected) setHistory(await listAffiliateImports(12).catch(() => []));
    else setHistory([]);
  };
  ...
  useEffect(() => {
    ...
    window.addEventListener('liv8:cloud-auth-changed', onCloud);
    return () => {
      window.removeEventListener(BOOK_UPDATED_EVENT, onBook);
      window.removeEventListener('liv8:cloud-auth-changed', onCloud);
    };
  }, []);
  ...
  const handleFile = async file => {
    ...
    try {
      const previous = getAffiliateBookRows();
      const parsed = parseAffiliateBookCSV(await file.text());
      const diff = compareAffiliateBooks(previous, parsed.rows);
      setLastDiff(diff);
      ...
      const changes = previous.length ? ` ${changeText(diff)}.` : ' First saved roster.';
      setMessage(result.cloud?.ok
        ? `Imported ${parsed.rows.length} affiliates and saved a cloud snapshot.${changes}`
        : `Imported ${parsed.rows.length} affiliates.${changes} Local + Mac copies are saved; connect LIV8 Cloud for cross-device sync.`);
      await refresh();
    } catch (e) {
  ...
  return <>
    ...
    {open && <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/70 p-4" onMouseDown={e => { if (e.target === e.currentTarget) setOpen(false); }}>
      <div className={`w-full max-w-4xl overflow-hidden rounded-2xl border shadow-2xl ${isDark ? 'border-cyan-500/20 bg-[#070c0f] text-white' : 'border-slate-200 bg-white text-slate-950'}`}>
        <div className={`flex items-start justify-between gap-4 border-b p-5 ${isDark ? 'border-white/10' : 'border-slate-200'}`}>
          <div>
            ...
            <h2 className="mt-1 text-xl font-bold">Import, compare, sync & restore</h2>
            <p className={`mt-1 max-w-2xl text-sm ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>Drop the newest Affiliate EXPAND CSV. LIV8 compares it with the saved book, updates the active CRM roster, and preserves a dated snapshot so worksheet changes never disappear.</p>
          </div>
        ...
        <div className="grid gap-4 p-5 md:grid-cols-[1.35fr_.85fr]">
          <div className="space-y-4">
            <button
              ...
              className={`flex min-h-[175px] w-full flex-col items-center justify-center rounded-2xl border-2 border-dashed p-6 text-center transition ${dragging ? 'border-cyan-400 bg-cyan-500/10' : isDark ? 'border-white/15 bg-white/[.025] hover:border-cyan-500/40' : 'border-slate-300 bg-slate-50 hover:border-cyan-400'}`}
            >
            ...
            <div className={`rounded-xl border p-4 ${isDark ? 'border-white/10 bg-white/[.025]' : 'border-slate-200 bg-slate-50'}`}>
              <div className="flex items-center justify-between gap-3">
                ...
                <div className={`text-right text-xs ${isDark ? 'text-slate-500' : 'text-slate-500'}`}><div>{fmt(meta.updatedAt)}</div><div>{meta.kind || ''}</div></div>
              </div>
              {lastDiff && <div className={`mt-3 grid grid-cols-3 gap-2 border-t pt-3 ${isDark ? 'border-white/10' : 'border-slate-200'}`}>
                <DiffStat label="Added" value={lastDiff.counts?.added || 0} isDark={isDark}/>
                <DiffStat label="Removed" value={lastDiff.counts?.removed || 0} isDark={isDark}/>
                <DiffStat label="Changed" value={lastDiff.counts?.changed || 0} isDark={isDark}/>
              </div>}
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className={`rounded-xl border p-4 ${cloud.connected ? 'border-emerald-500/25 bg-emerald-500/10' : isDark ? 'border-amber-500/25 bg-amber-500/10' : 'border-amber-300 bg-amber-50'}`}>
              <div className="flex items-center gap-2 font-semibold">{cloud.connected ? <Cloud className="h-4 w-4 text-emerald-400"/> : <CloudOff className="h-4 w-4 text-amber-400"/>}{cloud.connected ? 'LIV8 Cloud synced' : 'Cross-device sync not connected'}</div>
              <p className={`mt-2 text-xs ${cloud.connected ? 'text-emerald-200/80' : isDark ? 'text-amber-200/80' : 'text-amber-800'}`}>{cloud.connected ? 'This book follows liv8ent@gmail.com across the Mac Mini, browser, and work laptop.' : 'Your local and Mac copies still work. Connect liv8ent@gmail.com once on this device to share the same saved book everywhere.'}</p>
            ...
            <div className={`rounded-xl border ${isDark ? 'border-white/10' : 'border-slate-200'}`}>
              ...
              <div className="max-h-[320px] overflow-auto p-2">
                {!cloud.connected ? <div className={`p-3 text-xs ${isDark ? 'text-slate-500' : 'text-slate-500'}`}>Connect LIV8 Cloud to recall dated imports from any device.</div> : !history.length ? <div className={`p-3 text-xs ${isDark ? 'text-slate-500' : 'text-slate-500'}`}>Your first cloud import will appear here.</div> : history.map(item => <div key={item.id} className={`mb-1 rounded-lg p-2 ${isDark ? 'hover:bg-white/5' : 'hover:bg-slate-50'}`}>
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0"><div className="truncate text-xs font-medium">{item.filename || 'Affiliate book snapshot'}</div><div className={`mt-0.5 text-[11px] ${isDark ? 'text-slate-500' : 'text-slate-500'}`}><FileClock className="mr-1 inline h-3 w-3"/>{fmt(item.imported_at)} · {item.row_count} rows</div></div>
                  ...
                  {item.metadata?.diff && <div className={`mt-1 text-[10px] ${isDark ? 'text-cyan-300/70' : 'text-cyan-700'}`}>{changeText(item.metadata.diff)}</div>}
                </div>)}
              </div>
            </div>
          </div>
...
function DiffStat({ label, value, isDark }) {
  return <div className={`rounded-lg p-2 text-center ${isDark ? 'bg-white/[.035]' : 'bg-white'}`}><div className="text-lg font-bold">{value}</div><div className={`text-[10px] uppercase tracking-wide ${isDark ? 'text-slate-500' : 'text-slate-500'}`}>{label}</div></div>;
}
