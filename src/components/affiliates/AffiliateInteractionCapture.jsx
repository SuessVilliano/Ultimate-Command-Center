...
function contactList(payload) { return payload?.contacts || payload?.data?.contacts || payload?.data || []; }
function fieldValue(contact, needle) { const fields = contact.customFields || contact.customField || []; const found = fields.find(f => String(f.name || f.fieldKey || f.key || '').toLowerCase().includes(needle)); return String(found?.value || found?.fieldValue || '').trim(); }
function exactMatches(contacts, identifier) { const key = identifier.trim().toLowerCase(); return contacts.filter(c => [c.email, c.id, c.contactId, fieldValue(c, 'promoter')].some(v => String(v || '').trim().toLowerCase() === key)); }
...
export default function AffiliateInteractionCapture() {
  ...
  async function findContact() {
    ...
    try {
      ...
      if (bookMatches.length === 1) { const contact = bookMatches[0]; setMatch({ ...contact, promoterId:contact.id, matchSource:'book' }); setState('matched'); setMessage('Matched against the private Book View. Headless submission will independently verify this affiliate inside the sandbox CRM before writing.'); return; }
      if (bookMatches.length > 1) { setState('capture'); setMessage('Multiple Book View records matched. Use the affiliate primary email or correct the duplicate Promoter ID before continuing.'); return; }
      const res = await fetch(`${API_URL}/api/ghl/contacts?query=${encodeURIComponent(form.identifier.trim())}&limit=10`); const data = await res.json(); if (!res.ok) throw new Error(data.error || 'Contact lookup failed');
      const matches = exactMatches(contactList(data), form.identifier);
      if (matches.length !== 1) { setState('capture'); setMessage(matches.length ? 'Multiple exact contacts matched. Tighten the identifier before continuing.' : 'No exact contact matched. Use the primary email, CRM contact ID, or populated Promoter ID.'); return; }
      setMatch({...matches[0], name: matches[0].name || [matches[0].firstName, matches[0].lastName].filter(Boolean).join(' '), matchSource:'ghl'}); setState('matched');
  ...
  function saveForReview() { if (!match || !form.rawNotes.trim()) return setMessage('Match one contact and enter the interaction summary first.'); const item = { id: crypto.randomUUID(), createdAt:new Date().toISOString(), contactId:match.contactId || (match.matchSource === 'ghl' ? match.id : null), promoterId:match.promoterId || (match.matchSource === 'book' ? match.id : null), contactName:match.name || match.email, identifier:form.identifier, note, status:'review' }; const next = [item, ...queue]; setQueue(next); localStorage.setItem(QUEUE_KEY, JSON.stringify(next)); setState('review'); setMessage('Saved to the review queue. Nothing has been posted to HighLevel.'); }
  ...
  async function postHeadless() {
    ...
    try {
      const response = await fetch(`${API_URL}/api/affiliate/crm-note`, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ identifier:form.identifier, email:match.email || '', promoterId:match.promoterId || (match.matchSource === 'book' ? match.id : ''), note, source:`ACC FORM ${SANDBOX_FORM_ID}` }) });
      const data = await response.json().catch(() => ({})); if (!response.ok) throw new Error(data.error || `CRM write failed (${response.status})`);
      setState('posted'); setMessage(`Posted headlessly to the sandbox CRM after exact-match verification${data.contactEmail ? ` for ${data.contactEmail}` : ''}. Form reference: ${SANDBOX_FORM_ID}.`);
  ...
  return <section className="rounded-2xl border border-purple-500/20 bg-gradient-to-br from-[#100a18] to-[#071217] p-5">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><div className="text-xs uppercase tracking-[.18em] text-purple-300">Fast CRM hygiene · sandbox</div><h2 className="mt-1 text-xl font-bold text-white">Affiliate Interaction Capture</h2><p className="mt-1 text-sm text-slate-400">One affiliate · exact sandbox match · approval preview · headless CRM note</p></div><div className="rounded-lg border border-white/10 px-3 py-2 text-xs text-slate-400"><Clock3 className="mr-1 inline h-3.5 w-3.5"/>{queue.filter(x=>x.status==='review').length} awaiting review</div></div>
    <div className="mt-3 rounded-lg border border-cyan-500/20 bg-cyan-500/[.06] p-3 text-xs text-cyan-100"><ShieldCheck className="mr-1 inline h-3.5 w-3.5"/>Sandbox guardrail: headless writes require the sandbox API key + location ID and are independently exact-matched before any CRM note is created. Form reference <b>{SANDBOX_FORM_ID}</b>.</div>
